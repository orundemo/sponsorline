...
`sponsorline` organization and stores generated credentials in AWS Secrets
Manager.
...
|-------------|----------------------------|--------------|
| stage       | sponsorline-stage    | Provisioned  |
| prod        | sponsorline-prod     | Provisioned  |
| dev         | —                          | Not provisioned |
...
State is stored at:
```
s3://sourceplane-<env>/env/<env>/sponsorline/supabase/terraform.tfstate
```
...
```
sourceplane/sponsorline/supabase/stage
sourceplane/sponsorline/supabase/prod
```
