...
│ (sponsorline-stage, sponsorline-prod)  │
└─────────────────────────────────────────────────────┘
```
...
- **`cloudflare_hyperdrive_config`**: Hyperdrive gateway resource
  - **Name**: `{namespacePrefix}sponsorline-{environment}` (e.g., `sponsorline-stage`)
...
| `orgName` | string | `sponsorline` | Organization name (for tags and secret paths) |
| `owner` | string | `sponsorline` | Repository owner (for tags) |
| `repo` | string | `sponsorline` | Repository name (for tags and secret paths) |
| `namespace` | string | `sponsorline` | Namespace (for labels and naming conventions) |
...
1. **`supabase` component** (same repo)
   ...
   - This component reads credentials from the secret path: `sourceplane/sponsorline/supabase/{environment}`
...
- **Secret path**: `sourceplane/sponsorline/supabase/{environment}`
...
aws secretsmanager get-secret-value --secret-id sourceplane/sponsorline/supabase/stage
```

### Validate Syntax

```bash
cd /path/to/sponsorline
