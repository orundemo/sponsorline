...
Your backend holds one SponsorLine API key and **zero GitHub credentials**.
...
const sponsorLine = new SponsorLine({
  baseUrl: process.env.SPONSORLINE_API_URL!,
  token: process.env.SPONSORLINE_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await sponsorLine.integrations.issueGithubToken(ORG_ID, {
