...
// deleted; all wire I/O now goes through the typed `SponsorLine` resource
...
//   - `createClient` — constructs a `SponsorLine` against a target, wired
...
//     `Promise<T>` + thrown `SponsorLineError` into the same shape the
...
import { SponsorLine, SponsorLineError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = SponsorLine;
...
export function createClient(target: ApiTarget, token: string | null): SponsorLine {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new SponsorLine(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `SponsorLineError` subclasses surface as `{ ok: false }` carrying the
...
export async function wrap<T>(fn: () => Promise<T>): Promise<ApiResult<T>> {
  try {
    ...
    if (e instanceof SponsorLineError) {
