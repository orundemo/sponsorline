...
Part of the sponsorline runtime: a Cloudflare Worker deployed per
environment (`stage`, `prod`; `dev` is verify-only). Public entry point: `https://sponsorline-api-edge-{stage,prod}.orundemo.workers.dev`
...
- **web-console-next** — Next.js 15 + opennextjs/cloudflare delivery of the SponsorLine web console (per-environment, Workers + Static Assets)
