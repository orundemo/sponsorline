...
`sponsorline` — first-class TypeScript CLI for the SponsorLine control
...
This package is internal to the `sponsorline` monorepo and is not
...
```
sponsorline login    [--api-url=URL] [--token=BEARER]
sponsorline logout
sponsorline whoami
```
...
```
sponsorline org list
sponsorline org use <org-id>
sponsorline org members
sponsorline project list
```

Writes (Task 0101):

```
sponsorline org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
sponsorline project create <name> [--idempotency-key=KEY]
sponsorline env create <project-id> <name> [--idempotency-key=KEY]
sponsorline api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
sponsorline webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
sponsorline usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
sponsorline billing summary
sponsorline audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`sponsorline org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `sponsorline login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/sponsorline/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/sponsorline/config.json` (mode 0644, not a secret). Override
both via `SPONSORLINE_CONFIG_DIR` (used by tests).
