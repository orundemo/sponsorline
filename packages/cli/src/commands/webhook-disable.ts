// `sponsorline webhook disable <endpointId> [--reason=TEXT]` — Task 0115.
//
...
export async function webhookDisableCommand(
  ...
  if (endpointId === undefined || endpointId.length === 0) {
    throw new UsageError(
      "usage: sponsorline webhook disable <endpointId> [--reason=TEXT] [--idempotency-key=KEY] [--output=human|json]",
    );
  }
