// CLI error helpers. Translate `SponsorLineError` subclasses (from
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `sponsorline org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `sponsorline login` to refresh", mode);
  }
  if (err instanceof SponsorLineError) {
