...
describe("resolveConfigDir", () => {
  it("respects SPONSORLINE_CONFIG_DIR override", () => {
    const prev = process.env["SPONSORLINE_CONFIG_DIR"];
    process.env["SPONSORLINE_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["SPONSORLINE_CONFIG_DIR"];
      else process.env["SPONSORLINE_CONFIG_DIR"] = prev;
    }
  });
});
