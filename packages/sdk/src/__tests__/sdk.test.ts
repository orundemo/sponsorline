...
import { SponsorLine } from "../index.js";
import {
  BadRequestError,
  ConflictError,
  ForbiddenError,
...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new SponsorLine({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => SponsorLineError]> = [
  ...
  it("forward-compat: unknown error code → base SponsorLineError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(SponsorLineError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as SponsorLineError).code).toBe("future_quota_exceeded");
    }
  });
