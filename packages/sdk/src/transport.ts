// Base HTTP transport for the SponsorLine SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `SponsorLineError`
...
export interface ClientOptions {
  /**
   * Base URL of the SponsorLine api-edge worker, e.g.
   * `https://api.sponsorline.dev`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `SponsorLine` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "SponsorLine SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "SponsorLine SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
