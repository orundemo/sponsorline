...
SponsorLine TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the SponsorLine control plane API.

## Install (workspace)

This package is internal to the `sponsorline` monorepo. Add it as a
...
```ts
import { SponsorLine } from "@saas/sdk";

const client = new SponsorLine({
  baseUrl: "https://api.sponsorline.dev",
  auth: { kind: "bearer", token: process.env["SPONSORLINE_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `SponsorLineError`. Branch on
...
| `SponsorLineError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `SponsorLineError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new SponsorLine({ fetch })`.
